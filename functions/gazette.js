...
const allowedNamesFromFacts = (facts) => {
    ...
    return [cup.name, cup.finalsTeam1, cup.finalsTeam2, cup.finalsWinner, cup.prizeLabel].filter(Boolean);
};
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        'You write one Konoplay Gazette issue in the dry voice of a 1994 business-sim newspaper. Return JSON with keys headline, body, artKey. artKey must be one of crest, castle, gold, finals, registration. Use only the provided facts. Do not invent players, scores, dates, or prize amounts. Headline max 80 characters. Body 2-4 short sentences.'
                },
                {
                    role: 'user',
...
exports.generateGazette = functions.https.onCall(async (data, context) => {
    ...
    const facts = buildGazetteFacts(tournamentsSnap.val() || {});
    const template = composeGazetteIssue(facts);
    let issue = sanitizeGazetteIssue(template, facts);
